...
// findAssetURLs procura entre os assets da release os binários taskthing
// para o OS/ARCH atual e retorna suas URLs de download.
func FindAssetURLs(release *githubRelease) (string, bool) {
	suffix := assetSuffix()

	var remembrallURL string
	...
	for _, asset := range release.Assets {
		...
		regexRemembrall := regexp.MustCompile(`^taskthing-v\d+\.\d+\.\d+$`)
...
func DownloadAndReplaceBinary(assetURL, binDir, currentBin, oldBin string) error {
	...
	tmpFile, err := os.CreateTemp(binDir, "taskthing-update-*")
	if err != nil {
		_ = os.Rename(oldBin, currentBin)
...
var UpdateApplyCommand *cli.Command = &cli.Command{
	...
	Action: func(ctx context.Context, cmd *cli.Command) error {
		...
		if currentConfig.NeedsUpdate {
			...
			runner := func() {
				remembrallURL, ok := FindAssetURLs(release)
				if !ok {
				...
				case "windows":
					...
					binDir = filepath.Join(localAppData, "Programs", "taskthing")
					binaryName = "taskthing.exe"
